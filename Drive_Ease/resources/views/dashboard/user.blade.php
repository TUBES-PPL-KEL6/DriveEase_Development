        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8"> {{-- Padding dan jarak antar bagian --}}
            ...
            <div class="bg-white p-6 md:p-8 rounded-xl shadow-lg border border-gray-200">
                <div class="flex items-center mb-4">
                    ...
                    <p class="text-xl text-gray-700">Selamat datang, <span
                            class="font-extrabold text-blue-600">{{ auth()->user()->name }}</span>!</p>
                </div>

                <p class="text-gray-600 mb-6">Kelola pemesanan Anda, temukan kendaraan baru, dan nikmati kemudahan bersama
                    DriveEase.</p>
            ...
            <div class="bg-white p-6 md:p-8 rounded-xl shadow-lg border border-gray-200">
                <h2 class="text-xl lg:text-2xl font-semibold text-gray-800 mb-4 flex items-center">
                    <svg class="w-6 h-6 text-indigo-500 mr-2 flex-shrink-0" fill="none" stroke="currentColor"
                        viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.001 12.001 0 002.92 12c0 1.57.413 3.072 1.15 4.355L4.475 18H20.025l.325-1.645A12.001 12.001 0 0021.08 12c0-1.57-.413-3.072-1.15-4.355z">
                        </path>
                    </svg>
                    ...
                    <div class="overflow-x-auto rounded-lg border border-gray-200 shadow-sm">
                        <table class="min-w-full w-full text-sm">
                            <thead class="bg-slate-100 text-slate-600">
                                <tr>
                                    <th scope="col"
                                        class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider">Kendaraan
                                    </th>
                                    <th scope="col"
                                        class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider">Mulai
                                    </th>
                                    <th scope="col"
                                        class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider">Selesai
                                    </th>
                                    <th scope="col"
                            ...
                            <tbody class="bg-white divide-y divide-slate-200 text-slate-700">
                                <tr class="hover:bg-slate-50 transition-colors duration-150">
                                    <td class="px-4 py-3 whitespace-nowrap">
                                        ...
                                        <div class="text-xs text-slate-500">{{ $latestBooking->vehicle->category ?? '' }}
                                        </div>
                                    </td>
                                    <td class="px-4 py-3 whitespace-nowrap">
                                ...
                                @foreach ($bookings as $booking)
                                    <tr class="hover:bg-slate-50 transition-colors duration-150">
                                        <td class="px-4 py-3 whitespace-nowrap">
                                            ...
                                            <div class="text-xs text-slate-500">{{ $booking->vehicle->category ?? '' }}
                                            </div>
                                        </td>
                                        <td class="px-4 py-3 whitespace-nowrap">
                                        ...
                                        <td class="px-4 py-3 whitespace-nowrap">
                                            ...
                                            @endphp
                                            <span
                                                class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $rentalStatusClass }}">
                                                {{ $rentalStatusText }}
                                            </span>
                                        </td>
                                        <td class="px-4 py-3 whitespace-nowrap text-center">
                                            <a href="{{ route('user.rents.show', $booking->id) }}"
                                                class="text-blue-600 hover:text-blue-800 hover:underline text-xs font-medium inline-flex items-center">
            ...
            <div class="bg-white p-6 md:p-8 rounded-xl shadow-lg border border-gray-200">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    @forelse($vehicles as $vehicle)
                        <div
                            class="bg-gray-50 border border-gray-200 rounded-lg shadow-sm overflow-hidden transform transition duration-300 hover:scale-[1.02] hover:shadow-md">
                            <a href="{{ route('vehicles.show', $vehicle->id) }}" class="block">
                                @if ($vehicle->image_path)
                                    <img src="{{ asset('storage/' . $vehicle->image_path) }}" alt="{{ $vehicle->name }}"
                                        class="w-full h-48 object-cover">
                                @else
                                    <img src="https://placehold.co/400x300/E2E8F0/94A3B8?text=Gambar+Mobil"
                            ...
                            <div class="p-4">
                                <h3 class="font-semibold text-lg text-gray-800 mb-1">
                                    <a href="{{ route('vehicles.show', $vehicle->id) }}"
                                        class="hover:text-blue-600 transition">{{ $vehicle->name }}</a>
                                </h3>
                                <p class="text-gray-600 text-sm mb-2">{{ $vehicle->category ?? 'N/A' }} &bull;
                                    {{ $vehicle->location ?? 'N/A' }}</p> {{-- Menggunakan category dan location --}}
                                <p class="font-bold text-blue-600 text-xl mb-4">Rp
                                    {{ number_format($vehicle->price_per_day, 0, ',', '.') }} <span
                                        class="text-sm font-normal text-gray-500">/ Hari</span></p>
                                <a href="{{ route('vehicles.show', $vehicle->id) }}"
                                    class="block text-center bg-blue-600 text-white py-2.5 px-4 rounded-lg hover:bg-blue-700 transition duration-300 font-semibold shadow-sm">
                                    Lihat Detail
                                </a>
                            </div>
                        </div>
