...
<div class="bg-white p-8 rounded-2xl shadow-xl mt-12 border border-gray-200 max-w-7xl mx-auto">
    ...
    @if($completedBookings->isEmpty())
        <div class="flex flex-col items-center justify-center py-16">
            ...
            <h4 class="text-xl font-semibold text-gray-700 mb-3">Belum ada pemesanan yang perlu direview</h4>
            ...
            <div class="bg-blue-50 rounded-lg p-4 max-w-md">
                <p class="text-sm text-blue-700">
                    <strong>💡 Tips:</strong> Review yang konstruktif membantu pelanggan lain dan meningkatkan reputasi bisnis Anda.
                </p>
            </div>
        </div>
    @else
                    ...
                    @foreach($completedBookings as $booking)
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <div class="text-xs text-gray-500">{{ $booking->created_at ? $booking->created_at->format('d M Y') : '-' }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    <div class="w-8 h-8 bg-indigo-100 rounded-full flex items-center justify-center mr-3">
                                        <span class="text-indigo-600 font-medium text-sm">{{ substr($booking->user->name ?? 'U', 0, 1) }}</span>
                                    </div>
                                    <div>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900 font-medium">{{ $booking->vehicle->name ?? '-' }}</div>
                                <div class="text-xs text-gray-500">{{ $booking->vehicle->category ?? '-' }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900">{{ $booking->start_date ? \Carbon\Carbon::parse($booking->start_date)->format('d M Y') : '-' }}</div>
                                <div class="text-xs text-gray-500">sampai {{ $booking->end_date ? \Carbon\Carbon::parse($booking->end_date)->format('d M Y') : '-' }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                ...
                                @else
                                    <button type="button" onclick="openReviewModal('{{ $booking->id }}')" class="inline-flex items-center px-4 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors duration-200 shadow">
...
@if(!$completedBookings->isEmpty())
    @foreach($completedBookings as $booking)
        @if(!$booking->rentalReview)
            <div id="review-modal-{{ $booking->id }}" 
                 class="fixed inset-0 z-50 hidden overflow-y-auto"
                 aria-labelledby="modal-title-{{ $booking->id }}" 
                 role="dialog" 
                 aria-modal="true"
                        ...
                        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                            <div class="flex items-start justify-between mb-4">
                                <div class="flex items-center">
                                    ...
                                    <div class="ml-3">
                                        <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-title-{{ $booking->id }}">
                                            Review Pelanggan
                                        </h3>
                                        <p class="text-sm text-gray-500">Berikan penilaian untuk pelanggan ini</p>
                                    </div>
                                </div>
                                <button type="button" 
                                        onclick="closeReviewModal('{{ $booking->id }}')"
                            ...
                            <div class="bg-gray-50 rounded-lg p-4 mb-6">
                                <dl class="grid grid-cols-1 gap-2 text-sm">
                                    ...
                                    <div class="flex justify-between">
                                        ...
                                        <dd class="text-gray-900">{{ $booking->vehicle->name ?? '-' }}</dd>
                                    </div>
                                    <div class="flex justify-between">
                            ...
                            <form id="review-form-{{ $booking->id }}" 
                                  action="{{ route('rental.reviews.store', $booking) }}" 
                                  method="POST" 
                                  class="space-y-4">
                                @csrf
                                ...
                                <div>
                                    ...
                                    <div class="flex items-center space-x-1" id="star-rating-{{ $booking->id }}">
                                        @for($i = 1; $i <= 5; $i++)
                                            <button type="button" 
                                    ...
                                    <p class="mt-1 text-xs text-gray-500">Klik bintang untuk memberikan rating</p>
                                </div>

                                {{-- Comment --}}
                                <div>
                                    <label for="comment-{{ $booking->id }}" class="block text-sm font-medium text-gray-700 mb-2">
                                        Komentar
                                    </label>
                                    <textarea name="comment" 
                                              ...
                                              placeholder="Bagikan pengalaman Anda dengan pelanggan ini (opsional)..."
