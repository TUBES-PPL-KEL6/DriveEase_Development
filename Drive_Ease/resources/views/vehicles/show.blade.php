...
@section('content')
    <div class="max-w-4xl mx-auto bg-white shadow rounded p-6">
        ...
        @auth
            @if(auth()->user()->role === 'pelanggan')
                @php
                    $alreadyReviewed = $vehicle->reviews()->where('user_id', auth()->id())->exists();
                @endphp
                @if(!$alreadyReviewed)
                    <div id="review-form" class="mt-10 border-t pt-6">
                        <h3 class="text-lg font-semibold mb-4">Tulis Ulasan Anda</h3>
                        <form action="{{ route('reviews.store', $vehicle->id) }}" method="POST" class="space-y-3">
                            @csrf
                            <div>
                            ...
                            <div>
                                ...
                                <textarea name="comment" rows="3" class="border rounded px-3 py-2 w-full" required></textarea>
                            </div>
