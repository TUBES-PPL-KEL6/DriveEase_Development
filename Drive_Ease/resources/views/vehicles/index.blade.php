    ...
    <div class="bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 py-12 md:py-16">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <form method="GET" action="{{ route('vehicles.index') }}"
                class="bg-white p-6 rounded-xl shadow-2xl border border-gray-200 mb-10 md:mb-12">
                <div class="flex items-center mb-4">
                    <svg class="w-6 h-6 text-blue-500 mr-2 rtl:ml-2 rtl:mr-0" fill="none" stroke="currentColor"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-5">
                    <div>
                        ...
                        <div class="relative">
                            <div
                                class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none rtl:right-0 rtl:left-auto rtl:pl-0 rtl:pr-3">
                            ...
                            <input name="location" id="location" placeholder="Ketik lokasi..."
                    ...
                    <div>
                        ...
                        <input type="number" name="price_min" id="price_min" placeholder="Harga Min (Rp)"
                            value="{{ request('price_min') }}"
                            class="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-30 sm:text-sm text-gray-900 placeholder-gray-500 py-2.5"
                            min="0" />
                    </div>
                    <div>
                        <label for="price_max" class="sr-only">Harga Maksimum</label>
                        <input type="number" name="price_max" id="price_max" placeholder="Harga Max (Rp)"
                ...
                <div class="mt-6 text-center">
                    <button type="submit"
                        class="w-full sm:w-auto inline-flex items-center justify-center px-10 py-3 border border-transparent rounded-lg shadow-md text-base font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition duration-150 transform hover:scale-105">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-8">
                @forelse($vehicles as $vehicle)
                    <div
                        class="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden hover:shadow-2xl transition-all duration-300 ease-in-out flex flex-col group transform hover:-translate-y-1">
                        ...
                        <div class="p-5 flex-grow flex flex-col">
                            <div class="mb-2">
                                <p class="text-xs text-blue-500 font-medium uppercase tracking-wider">
                                    {{ $vehicle->category }}</p>
                                <h3 class="text-xl font-bold text-gray-800">
                                    <a href="{{ route('vehicles.show', $vehicle->id) }}"
                                        class="hover:text-blue-600 transition line-clamp-1">{{ $vehicle->name }}</a>
                                </h3>
                                <p class="text-xs text-gray-500 flex items-center mt-1">
                                    <svg class="w-3.5 h-3.5 mr-1 rtl:ml-1 rtl:mr-0 text-gray-400" fill="none"
                            ...
                            @if ($vehicle->average_rating)
                                <div class="flex items-center my-2">
                                    @for ($i = 1; $i <= 5; $i++)
                                        <svg class="w-4 h-4 {{ $i <= round($vehicle->average_rating) ? 'text-yellow-400' : 'text-gray-300' }}"
                                    ...
                                    @endfor
                                    <span
                                        class="ml-1.5 rtl:mr-1.5 rtl:ml-0 text-xs text-gray-500">({{ number_format($vehicle->average_rating, 1) }})</span>
                                </div>
                            @else
                                <div class="my-2 h-4">
                            ...
                            <p class="text-green-600 font-bold text-xl my-1">Rp
                                {{ number_format($vehicle->price_per_day, 0, ',', '.') }}<span
                                    class="text-sm font-normal text-gray-500">/hari</span></p>

                            <div class="mt-auto pt-4">
                                <a href="{{ route('vehicles.show', $vehicle->id) }}"
                                    class="btn-lihat-detail-kendaraan w-full inline-flex items-center justify-center px-4 py-2.5 border border-transparent rounded-lg shadow-md text-sm font-semibold text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition duration-150 transform hover:scale-105 group-hover:bg-blue-700">
                                    Lihat Detail & Pesan
                                    <svg class="ml-2 rtl:mr-2 rtl:ml-0 -mr-1 rtl:-ml-1 rtl:mr-0 h-5 w-5 transform transition-transform group-hover:translate-x-1 rtl:group-hover:-translate-x-1"
                    ...
                    <div class="col-span-1 sm:col-span-2 lg:col-span-3">
                        <div class="bg-white border-2 border-dashed border-gray-300 p-10 rounded-lg text-center shadow-sm">
                            ...
                            <p class="mt-1 text-sm text-gray-500">
                                Mohon maaf, kami tidak menemukan kendaraan yang sesuai dengan kriteria pencarian Anda. Coba
                                kata kunci atau filter lain.
                            </p>
                        </div>
                    </div>
                @endforelse
