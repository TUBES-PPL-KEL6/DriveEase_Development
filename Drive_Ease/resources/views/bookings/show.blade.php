<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="text-2xl lg:text-3xl font-bold text-gray-800">Detail Pemesanan</h2>
        </div>
    </x-slot>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-x-8 gap-y-6">
                        ...
                        <div class="lg:col-span-1 space-y-6">
                            <div class="aspect-w-16 aspect-h-12">
                                <img src="{{ $booking->vehicle->image_url ?? $booking->vehicle->image_path ?? 'https://placehold.co/600x450/E2E8F0/94A3B8?text=Gambar+'.urlencode($booking->vehicle->name) }}"
...
>>>>>>> Stashed changes
                                    ...
                                    <span class="ml-1.5 rtl:mr-1.5 rtl:ml-0 font-semibold text-sm">
                                        ...
                                        @elseif ($booking->status === 'konfirmasi') Terkonfirmasi
                                        @elseif ($booking->status === 'berjalan') Sedang Berjalan
                                        @elseif ($booking->status === 'selesai') Selesai
                                        @elseif ($booking->status === 'batal') Dibatalkan oleh Anda
                                        @elseif ($booking->status === 'tolak') Ditolak oleh Penyedia
                            ...
                            <div class="space-y-3">
                                ...
                                @if ($booking->status === 'selesai')
                                    @php
                                        ...
                                        $userReview = $booking->review()->where('user_id', auth()->id())->first();
                                    @endphp
                                    @if (!$userReview)
                                        <a href="{{ route('reviews.create', ['booking_id' => $booking->id, 'vehicle_id' => $booking->vehicle_id]) }}"
                                            ...
                                            <svg class="w-5 h-5 mr-2 rtl:ml-2 rtl:mr-0" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>
                                            Tulis Ulasan
                                        </a>
                                    @else
                                        <a href="{{ route('reviews.edit', $userReview->id) }}"
                                            ...
                                            <svg class="w-5 h-5 mr-2 rtl:ml-2 rtl:mr-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                            Edit Ulasan Anda
                                        </a>
                                    @endif
                                @endif
                                {{-- Tombol "Ajukan Perubahan" jika statusnya 'batal' atau 'konfirmasi' (jika > 24 jam)
                                     Fitur ini bisa kompleks, untuk saat ini kita sederhanakan. Jika 'batal', user mungkin harus booking ulang.
                                ...
                                <button type="button"
                                    ...
                                    <svg class="w-5 h-5 mr-2 rtl:ml-2 rtl:mr-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"></path></svg>
                                    Ajukan Perubahan
                                </button>
                                @endif
                                --}}
                        ...
                        <div class="lg:col-span-2 space-y-6">
                            ...
                            <div class="bg-gray-50 rounded-lg p-5 border border-gray-200 shadow-sm">
                                <h3 class="text-lg font-semibold text-gray-800 mb-3">Detail Pemesanan</h3>
                            ...
                            <div class="bg-gray-50 rounded-lg p-5 border border-gray-200 shadow-sm">
                                <h3 class="text-lg font-semibold text-gray-800 mb-3">Informasi Penyedia Jasa</h3>
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4">
                                    <div>
                                        <p class="text-xs text-gray-500">Nama Penyedia</p>
                                        {{-- Asumsi relasi vehicle->rental atau vehicle->owner ada --}}
                                        <p class="text-sm font-medium text-gray-700">{{ $booking->vehicle->rental->name ?? $booking->vehicle->owner->name ?? 'N/A' }}</p>
                                    </div>
                                    <div>
                                        <p class="text-xs text-gray-500">Email Kontak</p>
                                        <p class="text-sm font-medium text-gray-700">{{ $booking->vehicle->rental->email ?? $booking->vehicle->owner->email ?? 'N/A' }}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="bg-gray-50 rounded-lg p-5 border border-gray-200 shadow-sm">
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4">
                                    <div>
                                        ...
                                        <p class="text-sm font-medium text-gray-700">{{ $booking->vehicle->category ?? 'N/A' }}</p>
                                    </div>
                                    <div>
                                        <p class="text-xs text-gray-500">Lokasi Jemput</p>
                                        <p class="text-sm font-medium text-gray-700">{{ $booking->vehicle->location ?? 'N/A' }}</p>
                                    </div>
                                    <div>
                                        <p class="text-xs text-gray-500">Plat Nomor</p>
                                        <p class="text-sm font-medium text-gray-700">{{ $booking->vehicle->license_plate ?? 'N/A' }}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            @if ($booking->driver_id && $booking->driver)
                                <div class="bg-gray-50 rounded-lg p-5 border border-gray-200 shadow-sm">
                                    <h3 class="text-lg font-semibold text-gray-800 mb-3">Informasi Driver</h3>
                                    <div class="flex items-center gap-4">
                                        <div>
                                            <img src="{{ $booking->driver->photo_url ?? 'https://ui-avatars.com/api/?name='.urlencode($booking->driver->name).'&background=EBF4FF&color=7F9CF5' }}"
                                                 alt="Foto {{ $booking->driver->name }}" class="w-16 h-16 rounded-full object-cover shadow-sm">
                                        </div>
                                        <div class="text-sm">
                                            <p class="font-medium text-gray-700">{{ $booking->driver->name }}</p>
                                            @if($booking->driver->phone_number)
                                            <p class="text-gray-500">Telp: {{ $booking->driver->phone_number }}</p>
                                            @endif
                            ...
                            <div class="bg-yellow-50 border-l-4 border-yellow-400 text-yellow-800 p-4 rounded-md shadow-sm">
                                ...
                                <svg class="w-5 h-5 text-yellow-500 mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path></svg>
                                    <div>
                                        ...
                                        <p class="text-sm">Perubahan atau pembatalan pemesanan tidak dapat dilakukan kurang dari 24 jam sebelum tanggal mulai sewa.</p>
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content bg-white">
                    <div class="modal-header border-b border-gray-200">
                        <h5 class="modal-title text-lg font-medium text-gray-900" id="cancelBookingModalLabel">Konfirmasi Pembatalan Pemesanan</h5>
                    ...
                    <form action="{{ route('user.bookings.cancel', $booking->id) }}" method="POST">
                        ...
                        <div class="modal-body text-gray-700">
                            <p>Apakah Anda yakin ingin membatalkan pemesanan untuk <strong class="text-gray-800">{{ $booking->vehicle->name }}</strong>?</p>
                            <p class="text-xs text-gray-500 mt-1">Pembatalan ini tidak dapat diurungkan setelah dikonfirmasi.</p>
                            <div class="mt-4">
                        ...
                        <div class="modal-footer bg-gray-50 border-t border-gray-200">
                            ...
                            <button type="submit" class="btn btn-danger bg-red-600 hover:bg-red-700 text-white shadow-sm">Ya, Batalkan Pemesanan</button>
                        </div>
                    </form>
                </div>
            </div>
    ...
    <div class="modal fade" id="requestChangeModal" tabindex="-1" aria-labelledby="requestChangeModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content bg-white">
                <div class="modal-header border-b border-gray-200">
                    <h5 class="modal-title text-lg font-medium text-gray-900" id="requestChangeModalLabel">Ajukan Perubahan Pemesanan</h5>
                    <button type="button" class="btn-close text-gray-400 hover:text-gray-600" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form action="{{ route('user.bookings.request_change', $booking->id) }}" method="POST"> {{-- Buat route ini --}}
    {{-- @csrf
                    <div class="modal-body space-y-4 text-gray-700">
                        <p>Anda dapat mengajukan perubahan untuk pemesanan ini. Perubahan akan memerlukan konfirmasi ulang dari penyedia.</p>
                        <div>
                            <label for="edit_vehicle_id" class="block text-sm font-medium text-gray-700 mb-1">Kendaraan (jika ingin ganti)</label>
                            <select name="vehicle_id" id="edit_vehicle_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm bg-white text-gray-900">
                                @php $availableVehicles = \App\Models\Vehicle::where('available', true)->get(); @endphp
                                @foreach ($availableVehicles as $vehicle)
                                <option value="{{ $vehicle->id }}" @if ($vehicle->id == $booking->vehicle_id) selected @endif>
                                    {{ $vehicle->name }} (Rp {{ number_format($vehicle->price_per_day,0,',','.') }}/hari)
                                </option>
                                @endforeach
                            </select>
                        </div>
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="date" name="start_date" id="edit_start_date" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm bg-white text-gray-900" value="{{ \Carbon\Carbon::parse($booking->start_date)->format('Y-m-d') }}" required>
                            </div>
                            <div>
                                <label for="edit_end_date" class="block text-sm font-medium text-gray-700 mb-1">Tanggal Selesai Baru</label>
                                <input type="date" name="end_date" id="edit_end_date" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm bg-white text-gray-900" value="{{ \Carbon\Carbon::parse($booking->end_date)->format('Y-m-d') }}" required>
                            </div>
                        </div>
                        <div>
                            <label for="edit_side_note" class="block text-sm font-medium text-gray-700 mb-1">Catatan Perubahan (opsional)</label>
                            <textarea name="side_note" id="edit_side_note" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm bg-white text-gray-900 placeholder-gray-400" placeholder="Alasan perubahan, permintaan khusus...">{{ $booking->side_note_user_request ?? '' }}</textarea>
                        </div>
                    </div>
                    <div class="modal-footer bg-gray-50 border-t border-gray-200">
                        <button type="button" class="btn btn-light" data-bs-dismiss="modal">Batal</button>
                        <button type="submit" class="btn btn-primary bg-blue-600 text-white">Ajukan Perubahan</button>
                    </div>
                </form>
            </div>
        </div>
